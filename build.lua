...
local function getBinaryPaths()
    local repo = "lucas-montes/knowbase-core"
    local command = 'curl -s --fail-with-body "https://api.github.com/repos/lucas-montes/knowbase-core/releases/latest" | jq "[.assets[] | .browser_download_url]"'
...
local function getRelease(paths)
    for _,v in pairs(paths) do
       print(M.get_os_name()) 
    end
end

print(getRelease(getBinaryPaths()))
--local os = vim.fn.has('win64') == 1 and 'windows' or vim.fn.has('mac') == 1 and 'darwin' or 'linux'
--
--local binary_name = 'knowbase'
--local binary_extension = os == 'windows' and '.exe' or ''
--
--local url = string.format('https://github.com/%s/releases/download/%s/%s-%s-%s%s', repo, tag, binary_name, os, vim.fn.has('mac') == 1 and 'arm64' or 'x64', binary_extension)
--
--local dest_dir = vim.fn.stdpath('data') .. '/site/pack/vendor/start/' .. repo
--
--vim.fn.system({'curl', '-LJO', url, '-o', dest_dir .. '/' .. binary_name .. binary_extension})
--
---- Optionally, make the binary executable on Linux and macOS
--if os ~= 'windows' then
--    vim.fn.system({'chmod', '+x', dest_dir .. '/' .. binary_name .. binary_extension})
--end
